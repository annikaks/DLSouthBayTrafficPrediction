...
class StandardScaler:
    """
    Simple z-score scaler over (T, N, F).
    """
    def __init__(self):
        self.mean = None
        self.std = None
...
def load_raw_data(path: str) -> np.ndarray:
    """
    Load preprocessed PeMS-BAY-2022 from .npy, .npz, or .pkl.
    ...
    if ext == ".npy":
        ...
        data = _load_from_pkl(path)
    else:
        raise ValueError(f"Unsupported file extension '{ext}'. Use .npy, .npz, or .pkl.")
...
def train_val_test_split(data: np.ndarray):
    """
...
class TrafficDataset(Dataset):
    """
    Wrapper dataset for regression or classification.
    """
    def __init__(
        self,
        X: np.ndarray,
